...
class AugWrapper(nn.Module):
    ...
    def forward(self, images, prob=0., types=[], **kwargs):
        if random() < prob:
...
norm_class = nn.BatchNorm2d  # TODO: KOKO


def upsample(scale_factor=2):
    return nn.Upsample(scale_factor=scale_factor, mode='nearest')  # TODO: experiment here
...
class FCANet(nn.Module):
    def __init__(
        ...
        *,
        ...
        dct_weights = self.doubleget_dct_weights(
...
class Trainer():
    def __init__(
        ...
        **kwargs
        ...
        self.image_extension = "jpg" if num_chans == 3 else None
    ...
    def load_config(self):
        ...
        self.num_chans = config['num_chans']
    ...
    def set_dataset(self, dataset, num_workers):
        ...
        dataloader = DataLoader(self.dataset, num_workers=num_workers, batch_size=self.batch_size, drop_last=True, pin_memory=True)
        self.loader = cycle(dataloader)


    def set_data_src(self, folder, chans):
        from warnings import warn
        warn("set_data_src shouldn't be called")
        num_workers = default(self.num_workers, math.ceil(
            NUM_CORES / self.world_size))
        self.dataset = ImageDataset(folder, self.image_size, chans=chans, aug_prob=self.dataset_aug_prob)
        dataloader = DataLoader(self.dataset, num_workers=num_workers, batch_size=math.ceil(
            self.batch_size / self.world_size), drop_last=True, pin_memory=True)
        self.loader = cycle(dataloader)
